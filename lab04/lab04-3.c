...
void run_game(int computer_number); //added function declaration
...
int main(int argc, char *argv[])
{
    ...
    while (prompt == 'y')     /* This line does not contain an error */
    {
		...
		prompt = ask_to_play(played); //fixed typo playd -> played
    }
...
char ask_to_play(int played_before)
{
    ...
    if (!played_before)    /* This line does not contain an error */
    {
        ...
        //scanf(" %c", yes_or_no);
		scanf(" %c", &yes_or_no); //added & symbol
    } else
    {
        scanf(" %c", &yes_or_no);
    }
...
void run_game(int computer_number)
{
    ...
    while (!correct)    /* This line does not contain an error */
    {
        ...
        } else if (number == computer_number) // was previously: number = computer_number which is not a boolean argument
        {
            printf("\nThe number was %d!\n", computer_number);
            printf("\nYou guessed the number correctly!\n\n"
                   "Do you want to play again? ('y' for yes)\n  -> ");
            correct = 1;
        } else if (number < computer_number) // was previously: (number < computer_number); removed ';'
        {
